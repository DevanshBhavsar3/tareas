...
import (
	...
	testingPkg "github.com/DevanshBhavsar3/tareas/internal/testing"
...
func TestTodoRepository_CreateTodo(t *testing.T) {
	...
	test := testingPkg.NewTest(t)
	defer test.Cleanup(ctx)

	todoRepo := repository.NewTodoRepository(test.Server)
	...
	t.Run("create todo successfully", func(t *testing.T) {
		...
		payload := &todo.CreateTodoPayload{
			Title:       "Test Todo",
			Description: testingPkg.Ptr("Test todo description"),
			Priority:    testingPkg.Ptr(todo.PriorityHigh),
			DueDate:     &dueDate,
		}
	...
	t.Run("create todo with minimum required fields", func(t *testing.T) {
		...
		assert.Equal(t, todo.PriorityLow, result.Priority)
...
func TestTodoRepository_GetTodoByID(t *testing.T) {
	...
	t.Run("get todo by id successfully", func(t *testing.T) {
		result, err := todoRepo.GetTodoByID(ctx, userID, &todo.GetTodoByIDPayload{
			ID: testTodo.ID,
		})
		require.NoError(t, err)
		require.NotNil(t, result)
	...
	t.Run("get todo with wrong user id", func(t *testing.T) {
		...
		result, err := todoRepo.GetTodoByID(ctx, wrongUserID, &todo.GetTodoByIDPayload{
			ID: testTodo.ID,
		})
		assert.Error(t, err)
		assert.Nil(t, result)
	...
	t.Run("with canceled context", func(t *testing.T) {
		...
		result, err := todoRepo.GetTodoByID(canceledCtx, userID, &todo.GetTodoByIDPayload{
			ID: testTodo.ID,
		})
		assert.Error(t, err)
		assert.Nil(t, result)
...
func TestTodoRepository_DeleteTodo(t *testing.T) {
	...
	t.Run("delete todo successfully", func(t *testing.T) {
		err := todoRepo.DeleteTodo(ctx, userID, &todo.DeleteTodoPayload{
			ID: testTodo.ID,
		})
		require.NoError(t, err)
	...
	t.Run("with canceled context", func(t *testing.T) {
		...
		err := todoRepo.DeleteTodo(canceledCtx, userID, &todo.DeleteTodoPayload{
			ID: testTodo.ID,
		})
		assert.Error(t, err)
	})
...
func createTestTodos(t *testing.T, ctx context.Context, repo *repository.TodoRepository, userID string, count int) []*todo.Todo {
	...
	for i := range count {
		...
		payload := &todo.CreateTodoPayload{
			...
			Description: testingPkg.Ptr(fmt.Sprintf("Test todo description %d", i+1)),
			Priority:    testingPkg.Ptr(todo.PriorityHigh),
			DueDate:     &dueDate,
		}
